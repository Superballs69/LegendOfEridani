...
Less harmful, still crippling.
*/

/obj/item/weapon/frosttrap
	name = "Welcome mat"
	throw_speed = 2
	throw_range = 1
	gender = PLURAL
	icon = 'icons/obj/items.dmi'
	icon_state = "frosttrap0"
	randpixel = 0
	desc = "A mechanically activated welcome mat. High tech, and made to incapacitate more than to kill."
	throwforce = 0
	w_class = ITEM_SIZE_NORMAL
	origin_tech = list(TECH_MATERIAL = 1)
	matter = list(MATERIAL_STEEL = 18750)
...
/obj/item/weapon/frosttrap/proc/can_use(mob/user)
	return (user.IsAdvancedToolUser() && !issilicon(user) && !user.stat && !user.restrained())

/obj/item/weapon/frosttrap/user_unbuckle_mob(mob/user as mob)
	if(buckled_mob && can_use(user))
		user.visible_message(
			"<span class='notice'>\The [user] begins freeing \the [buckled_mob] from \the [src].</span>",
			"<span class='notice'>You carefully begin to free \the [buckled_mob] from \the [src].</span>",
...
/obj/item/weapon/frosttrap/attack_self(mob/user as mob)
	..()
	if(!deployed && can_use(user))
		user.visible_message(
			"<span class='danger'>[user] starts to deploy \the [src].</span>",
		...
		if (do_after(user, 25, src) && user.unEquip(src))
			user.visible_message(
				"<span class='danger'>\The [user] has deployed \the [src].</span>",
				"<span class='danger'>You have deployed \the [src]!</span>",
				"You hear a latch click loudly."
...
/obj/item/weapon/frosttrap/attack_hand(mob/user as mob)
	if(buckled_mob)
		user_unbuckle_mob(user)
	else if(deployed && can_use(user))
		user.visible_message(
...
/obj/item/weapon/frosttrap/proc/attack_mob(mob/living/L)
	...
	if(!L.apply_damage(10, BRUTE, target_zone, blocked, used_weapon=src))
		return 0
...
/obj/item/weapon/frosttrap/Crossed(AM as mob|obj)
	if(deployed && isliving(AM))
		var/mob/living/L = AM
		if(!MOVING_DELIBERATELY(L))
			L.visible_message(
...
/obj/item/weapon/frosttrap/on_update_icon()
	..()

	if(!deployed)
		icon_state = "frosttrap0"
	else
		icon_state = "frosttrap1"
